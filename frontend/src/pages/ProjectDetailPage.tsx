...
export const ProjectDetailPage: React.FC<Props> = ({ id, onBack }) => {
  ...
  if (isLoading) return <div className="p-8 space-y-6"><Skeleton className="h-10 w-1/3" /><div className="grid grid-cols-4 gap-4">{[1,2,3,4].map(i=><Skeleton key={i} className="h-24" />)}</div><Skeleton className="h-64" /></div>;
