      ...
      <table className="w-full text-left">
        ...
        <tbody className="divide-y divide-border-main/60">
          {data.map((row, idx) => (
            <tr
              key={idx}
              className="table-row hover:bg-elevated/60 transition-colors duration-fast ease-out"
              style={{ animationDelay: `${Math.min(idx, 10) * 30}ms` }}
            >
              {columns.map((col) => (
                <td
                  key={col.key}
                  className={`py-4 text-small ${col.align === 'right' ? 'text-right tabular-nums' : 'text-text-main'}`}
                >
                  {col.render(row)}
                </td>
              ))}
