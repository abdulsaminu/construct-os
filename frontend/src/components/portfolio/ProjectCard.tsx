...
export const ProjectCard: React.FC<Props> = ({ project, risk, onSelect }) => {
  ...
  return (
    <div 
      ...
      className="bg-surface rounded-2xl border border-border-main p-6 shadow-soft hover:-translate-y-0.5 transition-all duration-150 cursor-pointer group"
      role="button"
      tabIndex={0}
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold text-text-main group-hover:text-primary transition-colors">{project.name}</h3>
          <StatusBadge status={project.status} />
        </div>
        <span className={`px-3 py-1 rounded-lg text-xs font-bold ${getRiskStyles()}`}>Risk: {getRiskLabel()}</span>
      </div>

      <div className="grid grid-cols-3 gap-4 mb-6 text-sm">
        <div>
          <p className="text-text-dim text-xs mb-1">Budget</p>
          <p className="font-semibold text-text-main">{money(project.totalBudget)}</p>
        </div>
        <div>
          <p className="text-text-dim text-xs mb-1 flex items-center gap-1"><ListChecks size={12} /> Milestones</p>
          <p className="font-semibold text-text-main">{project.milestones.length}</p>
        </div>
        <div>
          <p className="text-text-dim text-xs mb-1 flex items-center gap-1"><Calendar size={12} /> Created</p>
      ...
      <div className="flex items-center justify-end text-primary text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity">
        Open Project <ArrowRight size={14} className="ml-1" />
      </div>
    </div>
  );
};
