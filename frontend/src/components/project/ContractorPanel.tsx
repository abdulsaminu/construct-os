...
export const ContractorPanel: React.FC<Props> = ({ project, contractors }) => {
  ...
  return (
    <div className="bg-surface rounded-2xl border border-border-main p-6">
      <h3 className="text-lg font-semibold text-text-main mb-4">Assigned Contractors</h3>
      {assignedContractors.length === 0 ? (
        <p className="text-sm text-text-dim">No contractors assigned.</p>
      ) : (
        <div className="space-y-3">
          {assignedContractors.map(c => {
            ...
            return (
              <div key={c.id} className="p-4 bg-elevated rounded-xl">
                <div className="flex items-center gap-3 mb-2">
                  <div className="p-2 bg-surface rounded-lg text-text-muted"><User size={16} /></div>
                  <div>
                    <p className="font-medium text-sm text-text-main">{c.name}</p>
                    <p className="text-xs font-mono text-text-dim">{c.payoutAddress.slice(0, 10)}...</p>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2 text-xs text-center mt-3 border-t border-border-main pt-3">
