...
export const MilestoneTimeline: React.FC<Props> = ({ milestones, contractors, onFund, onClaim, onSettle, isSettling, projectStatus }) => {
 ...
 if (m.settled) return <span className="text-caption font-semibold text-success flex items-center gap-1"><CheckCircle aria-hidden='true' size={16} /> On-Chain</span>;
 ...
 if (m.funded) return <button onClick={(e) => { e.stopPropagation(); onClaim(m.id, m.payeeId || ''); }} className="bg-warning/20 text-warning hover:bg-warning/30 px-4 py-3 rounded-8 text-caption font-bold transition-colors">Claim</button>;
 if (projectStatus !== 'completed') return <button onClick={(e) => { e.stopPropagation(); onFund(m.id); }} className="bg-primary hover:bg-primary-hover text-white px-4 py-3 rounded-8 text-caption font-bold transition-colors">Fund</button>;
    return null;
  };
