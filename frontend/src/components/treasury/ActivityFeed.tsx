...
export const ActivityFeed: React.FC<Props> = ({ entries, isLoading }) => {
  if (isLoading) return <Panel className="col-span-4"><SectionHeader title="Activity Feed" /><div className="space-y-3 mt-6">{[1,2,3,4].map(i=><Skeleton key={i} className="h-12 w-full" />)}</div></Panel>;
    ...
    <Panel className="col-span-4">
      ...
      <div className="mt-6 space-y-2">
        {feedItems.length === 0 ? (
          ...
          feedItems.map(item => (
            <div key={item.id} className="p-3 bg-elevated rounded-lg">
              <p className="text-sm font-medium text-text-main">{item.text}</p>
              <div className="flex justify-between mt-1">
                <p className="text-xs text-text-dim">{item.project}</p>
