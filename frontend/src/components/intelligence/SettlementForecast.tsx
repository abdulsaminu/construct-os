...
export const SettlementForecast: React.FC<Props> = ({ forecast, isLoading, projectMap }) => {
  if (isLoading || !forecast) return <Panel className="lg:col-span-4 col-span-12"><div className="space-y-4">{[1,2].map(i=><div key={i} className="bg-elevated rounded-12 h-24 animate-pulse" />)}</div></Panel>;
