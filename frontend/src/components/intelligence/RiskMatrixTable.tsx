...
export const RiskMatrixTable: React.FC<Props> = ({ risks, projectMap, isLoading }) => {
  ...
  if (isLoading) return <Panel className="col-span-12"><div className="space-y-3">{[1,2,3].map(i=><div key={i} className="bg-elevated rounded-12 h-14 animate-pulse" />)}</div></Panel>;
            ...
            <tbody className="divide-y divide-border-main/50">
              {entries.map(([id, r]) => (
                <tr key={id} className="hover:bg-elevated/50 transition-colors">
                  <td className="py-4 pr-4 text-small font-medium text-text-main whitespace-nowrap">
                    {projectMap[id] || `${id.substring(0, 8)}...`}
                  </td>
                  <td className="py-4 px-4 text-small text-text-muted font-mono">{r.scheduleRisk}</td>
                  <td className="py-4 px-4 text-small text-text-muted font-mono">{r.liquidityRisk}</td>
                  <td className="py-4 px-4 text-small text-text-muted font-mono">{r.fundingRisk}</td>
                  <td className="py-4 px-4 text-small text-text-muted font-mono">{r.contractorRisk}</td>
                  <td className="py-4 px-4 text-small font-bold text-text-main font-mono">{r.composite}</td>
                  <td className="py-4 pl-4"><CategoryBadge score={r.composite} /></td>
                  <td className="py-4 pl-4 text-caption text-text-dim">—</td>
                </tr>
              ))}
            </tbody>
          </table>
