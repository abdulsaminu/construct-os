      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-small text-text-muted mb-2">Notes</label>
              <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} rows={3} className="w-full bg-elevated border border-border-main rounded-12 p-3 text-small text-text-main outline-none focus:border-primary resize-none" placeholder="Internal notes..." />
            </div>
          </div>
        </div>
        ...
        <button 
          ...
          className="w-full bg-primary hover:bg-primary-hover text-white p-4 rounded-12 font-semibold transition-colors disabled:opacity-50"
        >
...
const InputField = ({ label, value, onChange, placeholder, required, type = 'text', isMono = false }: any) => (
  <div>
    <label className="block text-small text-text-muted mb-2">{label}</label>
    <input 
      type={type}
      required={required}
      value={value} 
      ...
      className={`w-full bg-elevated border border-border-main rounded-12 p-3 text-small text-text-main outline-none focus:border-primary transition-colors ${isMono ? 'font-mono' : ''}`}
    />
  </div>
);
