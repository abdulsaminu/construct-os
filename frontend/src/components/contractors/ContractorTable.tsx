  ...
  const columns = [
    ...
    { key: 'company', header: 'Company', render: (row: Contractor) => (
      <button onClick={() => onSelect(row)} className="flex items-center gap-3 text-left hover:text-primary transition-colors">
        <div className="p-2 bg-elevated rounded-lg text-text-muted"><User size={16} /></div>
        <div>
          <p className="font-medium text-text-main">{row.name}</p>
          <p className="text-xs text-text-dim">Primary Contractor</p>
        </div>
      </button>
    )},
    { key: 'wallet', header: 'Wallet', render: (row: Contractor) => (
      <div className="flex items-center gap-2">
        <span className="font-mono text-xs text-text-muted">{row.payoutAddress.slice(0, 6)}...{row.payoutAddress.slice(-4)}</span>
        <CopyButton text={row.payoutAddress} />
      </div>
    )},
    { key: 'date', header: 'Registered', render: (row: Contractor) => <span className="text-sm text-text-muted">{new Date(row.registeredAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span> },
    { key: 'projects', header: 'Projects', render: () => <span className="text-sm text-text-dim">—</span> }, // Backend doesn't expose count yet
  ];
